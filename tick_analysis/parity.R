...
df_in <- na.omit(cbind(EU_b_day, EG_a_day, GU_a_day))
df_out <- na.omit(cbind(EU_a_day, EG_a_day, GU_b_day))
d_parity_in <- (df_in$EG_a * df_in$GU_a - df_in$EU_b)*10000
d_parity_out <- (df_out$EG_a * df_out$GU_b - df_out$EU_a)*10000
plot(d_parity_in, ylim=c(-2,2))
